...
- **Bouton "Payer" sans réponse sur Safari iOS** (sauvegarde v7) : Sur `/payment`, le CTA ne déclenchait apparemment rien sur Safari iOS, alors que Chrome iOS fonctionnait. Hypothèse retenue (non confirmée DevTools) : `handlePay` était `async` et faisait `await createPayment(...).catch(() => {})` sur une mutation Convex. Quand la WS Convex est suspendue par Safari iOS (économie d'énergie, perte de focus, throttling cellulaire), la Promise reste **pending indéfiniment** — `.catch` ne se déclenche que sur rejet, pas sur pending. Conséquence : `navigate('/confirmation')` jamais atteint, spinner infini perçu comme "le bouton ne répond pas". Corrigé en passant `handlePay` en synchrone fire-and-forget : `void createPayment({...}).catch(() => {})` + navigate immédiat. Ajout préventif de `touchAction: 'manipulation'` et `WebkitTapHighlightColor: 'transparent'` sur les 3 boutons (Apple Pay, Google Pay, CTA Visa). Aucune logique Stripe dans ce code — le flow `/payment` est une **simulation démo** qui ne fait qu'écrire dans Convex `payments` puis naviguer. À reconfirmer via Safari Web Inspector connecté à l'iPhone si le bug persiste après deploy.
- **Bugs texte landing page + marquee logos** (sauvegarde v10) : (1) `TextReveal` dans `CtaFinal.tsx`, `Testimonials.tsx`, `Solution.tsx` : le trailing `' '` à l'intérieur d'un `span` `display:inline-block` est strippé par le browser (règle CSS standard). Fix : wrapper `<span>` non-animé contenant le `m.span` animé + le `' '` comme nœud texte frère — `{word}` est dans le `m.span`, le `' '` est APRÈS. (2) Marquee logos mobile (`Logos.tsx`) : le `m.div` flex prenait la largeur du parent `overflow-hidden` (= viewport), les items se compressaient (`flex-shrink:1` défaut) → gaps invisibles. Fix : `style={{ width: 'max-content' }}` sur le strip animé + `shrink-0` sur chaque item. Règle générale : tout strip de marquee animé doit avoir `width: max-content` et ses items `flex-shrink: 0`.
- **Marketing pages redesign + FonctionnalitesHero** (sauvegarde v9) : Refonte complète des pages secondaires. (1) `FonctionnalitesHero.tsx` créé dans `src/components/Fonctionnalites/` : composant hero animé avec `MagneticButton` (Emil Kowalski), `FeaturePill` anchors vers `#feature-0X`, `PaymentCard`/`FeedbackCard`/`DashboardCard` flottantes. Intégré dans `Fonctionnalites.tsx` en remplacement du hero précédent. `pt-28 md:pt-36` pour dégager la navbar fixe. (2) `Changelog.tsx` : dark timeline avec 5 entrées colorées par catégorie (`CATEGORY_CONFIG`), hero animé avec bar chart bundle size, CTA section. (3) `PressePage.tsx` : pitch card avec copie presse-papier (`sonner` toast), chiffres clés dark, 4 ressources, contact YG. (4) `AidePage.tsx` : accordion FAQ `AnimatePresence` single-open (`useState<string|null>(null)`), 3 catégories × 4 questions, tags populaires. `fontSize: 16` sur input pour éviter iOS zoom. (5) `SecuritePage.tsx` : certifications ACTIF, flow paiement 5 étapes numérotées, diagramme Client→Splitzy→Viva Wallet→IBAN avec badge TLS, transparence données 2 colonnes (✅/❌), 4 cards RGPD. (6) `package.json` : ajout `"sonner": "^2.0.7"` — manquait comme dépendance déclarée (existait en node_modules local mais absent du package.json → build Vercel échouait avec TS2307). (7) `Fonctionnalites.tsx` : suppression imports inutilisés `useScroll`/`useTransform` (TS6133 noUnusedLocals). Règle : toujours vérifier que les dépendances utilisées dans les pages marketing sont dans `package.json` — le build local peut passer si elles sont en node_modules transitive/orpheline, mais le build Vercel (fresh install) échouera.
- **Homepage sections 1.5-1.9 + blog routing** (sauvegarde v8) : Complétion du site marketing splitzy.fr. (1) `Stats.tsx` réécrit : section "Le problème" bg `#FAFAFA`, spring counters `useCountUp` via `useFadeInView`, TextReveal `whileInView`. (2) `Solution.tsx` créé : section "La solution" bg `#18181B`, 3 feature blocks avec visuels animés (phone mockup, dashboard KPI, QR scan), floating badge `animate={{ y: [0,-8,0] }}`, parallax scroll `useScroll`+`useTransform`. (3) `Testimonials.tsx` réécrit : perspective hover 3D via `useMotionValue`+`useSpring`, stagger entrance. (4) `PricingPreview.tsx` réécrit : 3 plans (Gratuit/Essentiel/Pro), card Essentiel proéminente `border: '2px solid #E8920A'` + badge. (5) `CtaFinal.tsx` réécrit : bg `#1A1A1A`, noise texture SVG fractalNoise à `opacity: 0.04`, `MagneticBtn` Emil Kowalski sur CTA. (6) `blogData.ts` créé : 10 articles complets système block-based (`p/h2/h3/quote/ul/ol/highlight/hr`), helper `getArticleBySlug`. (7) `BlogArticlePage.tsx` créé : route `/blog/:slug`, `Blocks` renderer, `RelatedArticles`, `ArticleCta`, breadcrumb, auteur. (8) `BlogPage.tsx` : tous les cards convertis de `<a href="#">` en `<Link to={/blog/:slug}>` + scroll-to-top. (9) `Footer.tsx` : scroll-to-top ajouté sur les 3 liens du bottom bar (CGU, Confidentialité, Mentions légales). Règle Framer Motion marketing : toujours `m.*` (jamais `motion.*`), toujours `whileInView` (jamais animations au mount), `useMotionValue`/`useSpring` importables depuis `framer-motion` main indépendamment de `LazyMotion`. Règle TypeScript : les strings contenant des apostrophes françaises (`l'`, `d'`, `qu'`) doivent utiliser des guillemets doubles comme délimiteurs.
