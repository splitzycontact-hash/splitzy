...
> **Règle deploy critique** : `Splitzy/convex/` ET `splitzy-client/convex/` déploient SUR LE MÊME projet Convex `splitzy-contact/splitzy` (prod = `mellow-chinchilla-481`). Le premier qui déploie écrase l'autre. **Avant tout `npx convex deploy --yes` depuis `Splitzy/`**, vérifier que `Splitzy/convex/` contient toutes les fonctions consumer-facing que splitzy.fr appelle (`restaurants.update`, `setSuspended`, `deleteAll`, `create` sans auth requise, etc.) — diff systématique avec `splitzy-client/convex/`. Si manquant, copier `splitzy-client/convex/*.ts` dans `Splitzy/convex/` AVANT de déployer. Voir incident 2026-05-23 ci-dessous.
...
- **Incident dashboard gérant cassé + ultra-review admin** (2026-05-23) : (1) Déploiement de `Splitzy/convex/` (monorepo) sur prod `mellow-chinchilla-481` a écrasé les fonctions splitzy-client utilisées par splitzy.fr. Symptôme : après login Clerk, le dashboard restaurateur ne marchait plus. Cause : `Splitzy/convex/restaurants.ts` manquait `update`, `setSuspended`, `deleteAll`, et son `create` exigeait `auth.getUserIdentity()` (que le client n'a pas). Fix : copie de `splitzy-client/convex/*.ts` → `Splitzy/convex/`, suppression des fichiers Splitzy-only orphelins (`crons.ts`, `diners.ts`, `e2e.ts`, `gdpr.ts`, `http.ts`, `sessions.ts`, `stripe.ts`, `stripeWebhooks.ts`), ajout de champs optionnels au schéma pour que l'admin compile (`kycStatus`, `siret`, `stripeAccountId`, `posProvider` sur restaurants, `managerReply` sur feedbacks), typage du retour de `dependencyStatus.list`, redeploy. (2) Ultra-review admin appliquée en parallèle (`Splitzy/apps/admin/src/`) : skeleton loading sur Restaurants + Bugs, `authEmail` sur tickets queries Support, labels FR sur Support, auto-seed feature flags via `useEffect` + ref, validation email regex dans Team invite, count restaurants dans bouton Communications, hint `⌘K` retiré de Sidebar, check `totpEnabled` retiré de `admin.ts` impersonation. **Leçon clé** : avant tout `npx convex deploy` depuis `Splitzy/`, diff systématique avec `splitzy-client/convex/`.
